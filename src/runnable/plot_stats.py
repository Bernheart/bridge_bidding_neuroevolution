...
def plot_stats(file_path=g.STATS_FILE_PATH):
    ...
    while flag:
        ...
        ax1.plot(smoothed_gen, moving_average(best_score[cut:], window), label="Best Score")
        ax1.plot(smoothed_gen, moving_average(avg_score[cut:], window), label="Average Score")
        ax1.plot(smoothed_gen, moving_average(worst_score[cut:], window), label="Worst Score")
        ...
        # ---- Figure 2: Length ----
        ax2.clear()
        ax2.plot(smoothed_gen, moving_average(best_length[cut:], window), label="Best Length")
        ax2.plot(smoothed_gen, moving_average(avg_length[cut:], window), label="Average Length")
        ax2.plot(smoothed_gen, moving_average(worst_length[cut:], window), label="Worst Length")
        ...
        # ax3.plot(smoothed_gen, moving_average(avg_length[cut:], window), label="Avg Average Score")
        # ax3.plot(smoothed_gen, moving_average(worst_length[cut:], window), label="Avg Worst Score")
        # ax3.set_xlabel("Generation")
        # ax3.set_ylabel(f"Score avg every {g.LOG_INTERVAL} gen")
        # ax3.set_title(f"Evolution Avg Score Over Generations [{g.VERSION}]")
        # ax3.legend()
        # ax3.grid(True)
